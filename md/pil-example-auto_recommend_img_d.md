---
title: pil example auto recommend img d (snippet)
date: 2020-03-02
tags: ["python"]
---
Python pil example 'auto recommend img d'
...
add_arg('--keyword', '-kw', default='车牌', help='输入要查询的关键词')
add_arg('--download_page', '-dp', default=1, type=int, help='希望下载的页数，每页60张图')
add_arg('--dir', default='E:\\test\\', help='下载图片保存的位置')
    ...
    if imgData['data']:
        for obj in imgData['data']:
            if obj and obj['objURL']:
                try:
                    urllib.urlretrieve(obj['objURL'],'E:/test/'+str(time.time())+'.jpg')
                    continue
