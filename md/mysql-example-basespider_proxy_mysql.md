---
title: mysql example basespider proxy mysql (snippet)
date: 2020-03-03
tags: ["python"]
---
Python mysql example 'basespider proxy mysql'

Functions in program: 
* `def mysql_insert(proxyes):`
* `def run():`
...
def fetch_66ip():
    ...
    try:
        ...
        url = "http://www.66ip.cn/nmtq.php?getnum=10&isp=0&anonymoustype=3&start=&ports=&export=&ipaddress=&area=1&proxytype=0&scrapy_js_api=66ip"
...
def run():
    ...
    proxyes = fetch_all()
    mysql_insert(proxyes)


def mysql_insert(proxyes):
    import MySQLdb
    conn = MySQLdb.connect(host="192.168.10.156", port=3306,
                           user="root", passwd="root",
                           db="DataCollectV1", charset=u"utf8")
    cur = conn.cursor()
    cur.execute("TRUNCATE net_proxy")
    conn.commit()
    for proxy in proxyes:
        cur.execute("INSERT INTO net_proxy set proxyip='%s';" % proxy)
    conn.commit()
    cur.close()
    conn.close()
